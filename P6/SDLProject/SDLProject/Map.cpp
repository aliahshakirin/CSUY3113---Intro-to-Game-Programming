...
int pondArr [] = {11 , 12, 13, 27, 27, 29, 43, 44, 45};

Map::Map(int width, int height, unsigned int *levelData, GLuint textureID, float tile_size, int tile_count_x, int tile_count_y, mapType type)
{
    this->width = width;
    this->height = height;
    this->type = type;
...
bool Map::IsPond(glm::vec3 position) {
    if (position.x < left_bound || position.x > right_bound) return false;
    if (position.y > top_bound || position.y < bottom_bound) return false;
    int tile_x = floor((position.x + (tile_size / 2)) / tile_size);
    int tile_y = -(ceil(position.y - (tile_size / 2))) / tile_size; // Our array counts up as Y goes down.
...
// add map function isTree/ isPond
